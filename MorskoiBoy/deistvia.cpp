...
void deistvia::corobli()
{
	while (corabli[0]!=4 && corabli[1]!=3 && corabli[2]!=2 && corabli[3]!=1) {
		...
		while (true) {
			...
			std::cout << "1) вверх";if (HodX + 1 - corabl > 0) { std::cout << " (вверх плыть можно)\n"; }else{ std::cout << " (вверх плыть нельзя)\n"; }
			std::cout << "2) вправо";if (HodY - 1 + corabl < 11) { std::cout << " (вправо плыть можно)\n"; }else{ std::cout << " (вправо плыть нельзя)\n"; }
			std::cout << "3) вниз";if (HodX - 1 + corabl < 11) { std::cout << " (вниз плыть можно)\n"; }else{ std::cout << " (вниз плыть нельзя)\n"; }
			std::cout << "4) влева";if (HodY + 1 - corabl > 0) { std::cout << " (влево плыть можно)\n"; }else{ std::cout << " (влево плыть нельзя)\n"; }
			std::cin >> povorot;
